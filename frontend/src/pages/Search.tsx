...
const Search = () => {
  ...
  const handleStarsChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectedStars(
      (prevStars) =>
        event.target.checked // Add or remove star rating from selectedStars based on checked state
          ? [...prevStars, starRating] // Add star rating to selectedStars if checked is true
          : prevStars.filter((star) => star !== starRating), // Remove star rating from selectedStars if checked is false
    );
  };

  const hanldeHotelTypeChange = (
      ...
      <div className='rounded-lg border border-slate-300 p-5 h-fit sticky top-10'>
        <div className='space-y-5'>
          ...
          <HotelTypeFilter
            ...
            onChange={hanldeHotelTypeChange}
          />
